...
for i = 1:length(P)
    text(x1(i) + 0.1, P(i), num2str(P(i), '%.2f'), ...
...
% Add numeric labels next to the stars for School H (offset right)
for i = 1:length(P)
    text(x2(i) + 0.1, P(i), num2str(P(i), '%.2f'), ...
        'FontSize', 10, 'FontName', 'Times', 'Color', 'b', 'HorizontalAlignment', 'left');
end
...
% set raven scores and standard errors for t-tests
P1p = R([1:5 11]);
P2p = R([6:10 12]);
