...
% Create the first subplot for School L and School H
subplot(1, 2, 1)

% Retrieve data for School L
P_L = RHO{1}([1:5 11]);        % Preference data for School L (grades and aggregate)
S_L = STANDARDERR{1}([1:5 11]); % Standard errors for School L
...
for i = 1:length(P_L)
    text(x1(i) + 0.1, P_L(i), num2str(P_L(i), '%.2f'), ...
        'FontSize', 13, 'FontName', 'Times', 'Color', 'r', 'HorizontalAlignment', 'left');
end

% Retrieve data for School H
...
for i = 1:length(P_H)
    text(x2(i) - 0.1, P_H(i), num2str(P_H(i), '%.2f'), ...
        'FontSize', 13, 'FontName', 'Times', 'Color', 'b', 'HorizontalAlignment', 'right');
end
...
% Create the second subplot for another data set
subplot(1, 2, 2)
...
T1st5thGrade_L_pencils = (P1p(1)-P1p(5))/sqrt(S1p(1)^2 + S1p(5)^2);
T1st5thGrade_H_pencils = (P2p(1)-P2p(5))/sqrt(S2p(1)^2 + S2p(5)^2);
...
TacrossSchool_1st_pencils = (P1p(1)-P2p(1))/sqrt(S1p(1)^2 + S2p(1)^2);
TacrossSchool_5th_pencils = (P1p(5)-P2p(5))/sqrt(S1p(5)^2 + S2p(5)^2);
TacrossSchool_Tot_pencils = (P1p(6)-P2p(6))/sqrt(S1p(6)^2 + S2p(6)^2);
...
TT = [T1st5thGrade_L_pencils
    T1st5thGrade_H_pencils;
T1st5thGrade_L_pens ;
T1st5thGrade_H_pens;

TacrossSchool_1st_pencils;
TacrossSchool_5th_pencils ;
TacrossSchool_Tot_pencils ;
...
section_5_2_1 = array2table([[P1p(1);P2p(1);P1pp(1);P2pp(1)], [P1p(5);P2p(5);P1pp(5);P2pp(5)], Pvalue(1:4)] , ...
    "RowNames", {'Pencils - School L', 'Pencils - School H', 'Pens -School L', 'Pens - School H'}, "VariableNames",{'1st','5th','pvalue'})

section_5_2_2 = array2table([[P1p(1);P1p(5);P1p(6);P1pp(1);P1pp(5);P1pp(6)], [P2p(1);P2p(5);P2p(6);P2pp(1);P2pp(5);P2pp(6)], Pvalue(5:end)], ...
    "RowNames", {'Pencils - 1st', 'Pencils - 5th', 'Pencils - aggregate', 'Pens - 1st', 'Pens - 5th', 'Pens - aggregate'}, "VariableNames",{'School L','School H','pvalue'})
