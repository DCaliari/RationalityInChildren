...
BOOTSTRAP2 = cell(size(N,1),1);
B = 10000;
for z=1:size(N,1)
    BOOTSTRAP2{z,1} = zeros(B,1);
end

for z = 1:size(N,1) % loop over all samples
for b = 1:B
p = [CORR(z,1),CORR(z,2),CORR(z,3)]; % multinomial distribution
...
BOOTSTRAP2{z,1}(b,1) = ( xu(3) - xu(1)*(1 - xu(3)) )/( xu(3) + xu(1)*(1 - xu(3)) ); % index of correlation neglect
...
for z=1:size(N,1)
STANDARDERR2(z,1) = std(BOOTSTRAP2{z,1});
end
