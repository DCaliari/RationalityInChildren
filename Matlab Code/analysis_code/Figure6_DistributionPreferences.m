...
subplot(1,2,1)
b1 = bar(cat1,preferences{1}(11:12,:));
ylim([0 0.3])
...
b1(2).FaceColor = 'b';
subplot(1,2,2)
b2 = bar(cat2,preferences{2}(11:12,:));
set(gca,'FontName','Times')
ylim([0 0.3])
legend('School L','School H')
set(gca,'FontName','Times')
...
set(gcf, 'PaperSize', [20 8.5]);  % Width and height of the paper

% Adjust the position and size of the plot to fill the entire page
set(gcf, 'PaperPosition', [0 0 20 8.5]);  % [left bottom width height]

% Adjust the figure window size to match the paper size (optional)
set(gcf, 'Units', 'inches');
set(gcf, 'Position', [1 1 20 8.5]);  % [left bottom width height]
