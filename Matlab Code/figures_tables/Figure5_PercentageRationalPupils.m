...
for i = 1:length(group)
    % Mark attrition if the response value is NaN and the group is covered
...
win = double(win); % Convert logical array to double
win(esito == 0) = 99; 

% Filter new data to exclude attrition
...
% Determine failing students for the coin drop game
fail_corr = (corr1_2w + corr2_2w + corr3_2w) > 1 | ...
            (corr1_2w + corr2_2w + corr3_2w) == 0; % Identify failures
...
% Create second subplot for Class results
subplot(1, 2, 2)
...
%% Load additional datasets for further analysis
...
clear nn; % Clear nn to avoid conflicts
for i = 1:5
    nn(1, i) = sum(class == i & (school == 1 | school == 4)); % Count students by class
end
nn = repmat(nn, 3, 1); % Replicate counts for all categories

% Define grade categories for the plot
