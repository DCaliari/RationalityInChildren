...
covered = zeros(length(group), 1);      % Initialize covered as zeros
for i = 1:length(group)            % Loop through all groups
    covered(i, 1) = ismember(group(i), groups_pachinko2ndwave); % Check if in second wave
end
covered = logical(covered);        % Convert to logical array

%%% Dummy variable for attrition
attrition = zeros(length(group), 1);    % Initialize attrition as zeros
for i = 1:length(group)            % Loop through all groups
    attrition(i, 1) = isnan(ind1_2w(i)) & covered(i); % Check for attrition criteria
end
...
win = (threes_twos == 1 & esito < 5 & esito > 0) | ...
      ...
      (twos_ones == 1 & esito < 8 & esito > 0);  % Define win conditions
win = double(win);                 % Convert to double type
win(esito == 0) = 99;              % Assign 99 to win where esito is 0

% Determine the newdata variable
newdata = covered & ~attrition;        % Create data set excluding attrition
...
%% Fit estimates correlated using optimization
...
for z = 1:size(N, 1)                % Loop over all samples
    for b = 1:B                      % Loop for bootstrap samples
        ...
        % Optimize the correlation estimates
...
for z = 1:size(N, 1)         % Loop through each size of N
...
for i=1:length(N)
  X(:,:,i) =  [round(IND(i,:)'.*N(i)),round(CORR(i,:)'.*N(i))];
    CHI(i, 1) = (  ( (X(1,1,i)-X(1,2,i))^2 )/ (X(1,1,i)+X(1,2,i))  +  ...
                ...
                ( (X(3,1,i)-X(3,2,i))^2 )/ (X(3,1,i)+X(3,2,i))  );
